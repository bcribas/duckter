...
for((k=0;k<ITERACOES;k++)); do
  printf "$k" >&2
  addchaves 1000 $i 500 > /tmp/tt
  for((i=0;i<10;i++)); do printf "dump keys\ndump tags\n";done >> /tmp/tt
  shuf /tmp/tt

  gentags $((100+RANDOM%100)) $i
  taghit 1000 > /tmp/tt
  for((i=0;i<10;i++)); do printf "dump keys\ndump tags\n";done >> /tmp/tt
  shuf /tmp/tt
  buscatags $((15+RANDOM%10)) > /tmp/tt
  echo list trending top $((15+RANDOM%10)) >> /tmp/tt
  echo list trending bottom $((15+RANDOM%10)) >> /tmp/tt
  buscatags $((15+RANDOM%10)) >> /tmp/tt
  shuf /tmp/tt
  echo '.' >&2
done
