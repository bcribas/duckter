...
for((k=0;k<ITERACOES;k++)); do
  printf "$k" >&2
  addchaves $((300 + RANDOM%700)) $i $((3 + RANDOM%500)) > /tmp/tt
  shuf /tmp/tt

  gentags $((800+RANDOM%300)) $i
  taghit $((2000 + RANDOM%5000))  > /tmp/tt

  addchaves 30 $k 1000 >> /tmp/tt

  for((i=0;i<10;i++)); do
    if (( RANDOM %2 == 0)); then
      echo list trending top 2
    else
      echo list trending bottom 2
    fi
  done >> /tmp/tt
  shuf /tmp/tt
  buscatags 1
